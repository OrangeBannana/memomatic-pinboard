...
- **Touchscreen regression (#25, OPEN — fix attempt awaiting hardware test).** On-device touch reverted to an earlier broken state. This branch (`issue-25-touch-regression-fix`) implements the fix for the prime suspects: `install.sh` reverts `gpu_mem=16` → `gpu_mem=64` (and remediates the already-written line on provisioned devices), `pinboard-touch.service` now logs compile/unbind results to the journal instead of swallowing them (a silently-failing `gcc` meant every touch fell back to the fixed (240, 100) coordinate), and `app/touch_diag.sh` (new) produces a one-shot triage report — run `sudo sh /home/memomatic/pinboard/app/touch_diag.sh` on the Pi. **Test plan + decision tree: [docs/touch-regression-25.md](docs/touch-regression-25.md).** Background leads, in case the gpu_mem revert is not it:
  1. **`gpu_mem=16`** — the fbcp-ili9341 "safe build" drives the TFT via GPU DMA. Starving the GPU to 16 MB may change fbcp's inter-frame DMA timing, which `spi_touch_read.c` busy-waits on to read the ADS7846 in the ~2 ms gap. If that window shifts, reads corrupt or return `err`. This branch reverts to `gpu_mem=64`.
  2. **`dtoverlay=disable-bt`** — frees the PL011 UART and remaps it; confirm it doesn't disturb the SPI0/GPIO7/GPIO17 pins the touch system depends on. Intentionally left in place on this branch (one boot variable at a time); the issue #1 Bluetooth branch removes it, so testing that branch doubles as this experiment.
  Also re-verify: `pinboard-touch.service` is active, the `ads7846` unbind succeeded, and `spi_touch_read` recompiled (it's built on the Pi by `ExecStartPre`; the journal now says `compiled OK` or an explicit `ERROR:`). Re-run `sudo python3 app/raw_touch.py` 4-corner test if coordinates are merely off rather than absent.
