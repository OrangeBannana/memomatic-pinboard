...
- **Remote access — hosted guest + owner site (#87)** — a Cloudflare Worker relay (`cloud/`: Worker + D1 + R2, deployed from the owner's CF account) hosts a public guest upload site (gated by a short per-event code) and an owner settings site (behind Cloudflare Access). The Pi runs `pinboard-cloudsync.service` (`app/cloud_sync.py`), a standalone **outbound-only** agent (NOT a FastAPI lifespan task — 0.92 ignores it) that polls the relay, ingests guest images/messages through the existing pipeline (`save_upload` + push-next/pending), applies a small set of basic settings via the local `PATCH /api/settings`, pushes current settings up, and acks. Home network is never exposed inbound; LAN `/admin`/`/guest`/`/frame` are unchanged. Built on the `remote-access` integration branch across staged PRs (#88–#92); full runbook + test plan in [docs/remote-access.md](docs/remote-access.md). Replaced the abandoned Bluetooth-guest approach (old #1).
...
- **Touchscreen regression (#25, root cause found 2026-06-11 — fix awaiting hardware test).** On-device diagnosis disproved the old suspects: the device's `/boot/firmware/config.txt` never received #4's changes at all (no `gpu_mem`, no `disable-bt`; `vcgencmd` reports `gpu=64M`), and the journal showed touch *partially* working — real coordinates ~2 of 9 touches, fixed fallback (240, 100) otherwise. **Actual root cause (measured):** `spi_touch_read`'s TA-detection spin (5 M uncached MMIO reads) takes 0.33–0.7 s when fbcp is idle — i.e. whenever the screen is static, which is most of the time on a 15 s slideshow — straddling `touch_bridge.py`'s 0.5 s subprocess timeout. Each touch was a coin flip: killed → fallback; fbcp active → fast TA path → success. The fallback coordinate deliberately toggles the menu, which masked the failure during earlier validation, and the richer menu later turned fallback taps into wrong-button presses (e.g. the spontaneous `slideshow_mode` flip to `memes`). **Fix on this branch:** detection window cut to 250 k iterations (~16–50 ms — an idle bus is safe to read immediately; that's the whole point), plus one retry in `_get_coords()` with the failure reason logged. The `gpu_mem=64` install.sh remediation is kept as defence for devices that were actually provisioned with `gpu_mem=16`. Diag tool: `sudo sh /home/memomatic/pinboard/app/touch_diag.sh`. Test plan: [docs/touch-regression-25.md](docs/touch-regression-25.md). If coordinates land but are *offset*, that's calibration, not this bug — re-run `sudo python3 app/raw_touch.py` and update the `CAL_*` constants.
